...
export function parseInboundMasivoExcel(buffer: ArrayBuffer): MasivoParseResult {
  ...
  if (aoa.length < 2) {
    return { headers: [], previewRows: [], batches: [], totalRows: 0, errors: ['El archivo no tiene suficientes filas. Se necesita al menos 1 fila de encabezado y 1 de datos.'] };
  }
  ...
  if (headerRowIdx === -1) {
    return { headers: [], previewRows: [], batches: [], totalRows: 0, errors: ['No se encontró una fila de encabezados con al menos 2 columnas.'] };
  }
  ...
  if (headers.length < 1) {
    return { headers: [], previewRows: [], batches: [], totalRows: 0, errors: ['No se detectaron columnas en el archivo.'] };
  }
