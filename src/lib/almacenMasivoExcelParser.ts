...
export function parseAlmacenMasivoExcel(buffer: ArrayBuffer): MasivoParseResult {
  ...
  for (let i = headerRowIdx + 1; i < aoa.length; i++) {
    ...
    for (let j = 0; j < headers.length; j++) {
      ...
      if (headers[j] === 'Cantidad Unidades' && typeof val === 'number') {
        obj[headers[j]] = Math.abs(val);
      } else if (headers[j] === 'Cantidad Almacenaje' && typeof val === 'number') {
        obj[headers[j]] = Math.abs(val);
      } else {
        obj[headers[j]] = val;
      }
    ...
    // Zona Almacenaje unificación: todo a una sola zona "Almacen"
    const zonaKey = 'Zona Almacenaje';
    if (obj[zonaKey] !== undefined && obj[zonaKey] !== null) {
      obj[zonaKey] = String(obj[zonaKey]);
    } else {
      obj[zonaKey] = 'Almacen';
    }
