...
import type { CostoOutboundColumna, CostoOutboundFila, ColumnType, FormulaConfig, ZonaColumnaDinamica, ZonaCeldaFormula } from '@/types/costos_outbound';
...
import CostosOutboundTable from './components/CostosOutboundTable';
...
import {
  useMasivoResumen, useArticuloResumen, useZonaCompaniaResumen, useZonaArticuloMensual,
  useOutboundClusterCompaniaResumen,
...
type ModalState = { open: false } | { open: true; editing: CostoOutboundColumna | null };

export default function CostosOutboundPage() {
  const [columnas, setColumnas] = useState<CostoOutboundColumna[]>([]);
  const [filas, setFilas] = useState<CostoOutboundFila[]>([]);
  ...
  const { data: resumenCompleto, loading: resumenLoading, load: loadResumen } = useArticuloResumen();
  ...
  const loadData = useCallback(async () => {
    ...
    const [
      ...
      supabase.from('costos_outbound_columnas').select('*').order('orden'),
      supabase.from('costos_outbound_operacion').select('*').order('orden'),
    ...
    const cols = (colData as CostoOutboundColumna[]) ?? [];
    const rows = (filData as CostoOutboundFila[]) ?? [];
    setColumnas(cols);
    setFilas(rows);
    ...
    // ── Build areasData for formula engine ──
  ...
  const { clusters, loadClusters } = useZonaClusters('costos_outbound_clusters');
  useEffect(() => { loadData(); loadMasivo(); loadClusters(); }, [loadData, loadMasivo, loadClusters]);

  // Cargar resumen cuando se selecciona tab de artículos/zonas
  useEffect(() => {
  ...
  const handleSaveColumn = async (data: { nombre: string; tipo: ColumnType; opciones: string[]; formula?: FormulaConfig }) => {
    ...
    if (isEditing && modalState.editing) {
      await supabase.from('costos_outbound_columnas').update(payload).eq('id', modalState.editing.id);
    } else {
      await supabase.from('costos_outbound_columnas').insert({ ...payload, orden: columnas.length });
    }
  ...
  const handleDeleteColumn = async (id: string) => {
    ...
    await supabase.from('costos_outbound_columnas').delete().eq('id', id);
    setColumnas(prev => prev.filter(c => c.id !== id));
  };

  // ── Rows ──
  const handleAddFila = async () => {
    const { data: newFila } = await supabase.from('costos_outbound_operacion').insert({ proceso: 'Nuevo proceso', subproceso: '', valores: {}, orden: filas.length }).select().maybeSingle();
    if (newFila) setFilas(prev => [...prev, newFila as CostoOutboundFila]);
  };
  ...
  const handleUpdateFila = useCallback(async (id: string, field: string, value: string | number) => {
    ...
    await supabase.from('costos_outbound_operacion').update({ [field]: value }).eq('id', id);
    setSavingId(null);
  }, []);
  ...
  const handleUpdateCell = useCallback(async (id: string, columnaId: string, value: string | number) => {
    ...
    await supabase.from('costos_outbound_operacion').update({ valores: { ...fila.valores, [columnaId]: value } }).eq('id', id);
    setSavingId(null);
  }, [filas]);

  const handleDeleteFila = async (id: string) => {
    await supabase.from('costos_outbound_operacion').delete().eq('id', id);
  ...
  const handleSaveRowFormula = useCallback(async (rowId: string, colId: string, formula: FormulaConfig) => {
    ...
    await supabase.from('costos_outbound_operacion').update({ formulas: newFormulas }).eq('id', rowId);
  }, [filas]);
  ...
  if (loading) {
    return (
      <AppLayout title="Costos Outbound" subtitle="Cargando módulo...">
  ...
  return (
    <AppLayout
      title="Costos Outbound"
      subtitle="Datos masivos de volúmenes + matriz de costos outbound con fórmulas"
      actions={
        <div className="flex items-center gap-2">
          <button onClick={() => setShowUpload(true)} className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-semibold text-slate-800">Datos masivos de volúmenes</h3>
              <p className="text-xs text-slate-400 mt-0.5">Archivo cargado tal cual, sin transformaciones</p>
            </div>
            {hasMasivo && (
              <span className="text-xs px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 font-medium">{fmt(masivoData!.totalRegistros)} registros</span>
            )}
          </div>

          {!hasMasivo ? (
            <div className="px-6 py-12 flex flex-col items-center gap-4">
              <div className="w-14 h-14 flex items-center justify-center rounded-full bg-amber-50">
                <i className="ri-database-2-line text-2xl text-amber-400" />
              </div>
              <div className="text-center max-w-sm">
                <p className="text-slate-700 font-semibold text-sm">Sin datos masivos</p>
                <p className="text-slate-400 text-xs mt-1">Carga un archivo Excel con la plantilla masiva para ver el resumen y usarlo en fórmulas.</p>
              </div>
              <button onClick={() => setShowUpload(true)} className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">
  ...
  return (
    <button onClick={() => onClick(tab)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors cursor-pointer whitespace-nowrap flex items-center gap-1.5 ${isActive ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
      ...
      {badge !== undefined && badge > 0 && (
        <span className={`ml-1 px-1.5 py-0.5 rounded-full text-[10px] ${isActive ? 'bg-white/20 text-white' : 'bg-emerald-100 text-emerald-700'}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard icon="ri-database-2-line" iconColor="text-amber-500" bg="bg-amber-50" label="Total registros" value={fmt(data.totalRegistros)} sub="filas cargadas" />
...
function ArticuloResumenTable({ data, loading, globalTotals }: { data?: ArticuloResumenRow[]; loading: boolean; globalTotals?: { totalMov: number; totalUnid: number; totalCount: number } }) {
  ...
  if (loading) return <div className="flex items-center justify-center py-16"><div className="flex flex-col items-center gap-3"><div className="w-8 h-8 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" /><p className="text-xs text-slate-400">Calculando resumen por artículo...</p></div></div>;
  const rows = data ?? [];
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="Buscar por código o descripción..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-100 focus:border-emerald-300 outline-none bg-white text-slate-700 placeholder:text-slate-400" />
        </div>
        <ExportMenu
          filenameBase="resumen-articulos"
          getExportData={() => {
            ...
            const exportRows = sorted.map(row => [
              ...
              row.meses_distintos,
              row.prom_movimientos_mes.toFixed(2),
              row.prom_unidades_mes.toFixed(2),
            ]);
...
// ── Zona Resumen Table (con pestañas por zona) ────────────────────────────

function ZonaResumenTable({ data, loading, globalTotals, formulaCtx, clusters, onClustersChange }: { data?: ZonaResumenRow[]; loading: boolean; globalTotals?: { totalMov: number; totalUnid: number; totalCount: number }; formulaCtx: FormulaContext; clusters: ZonaCluster[]; onClustersChange: () => void }) {
  const fmt = (n: number) => new Intl.NumberFormat('es-CO', { maximumFractionDigits: 0 }).format(n);
  const fmtDec = (n: number) => new Intl.NumberFormat('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n);
  const fmtPct = (n: number) => new Intl.NumberFormat('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n);
  const [search, setSearch] = useState('');
  ...
  const [prevZone, setPrevZone] = useState<string>(rows[0]?.zona ?? '');
  const { data: companiaData, loading: companiaLoading } = useZonaCompaniaResumen(activeZone);
  const { data: _articuloMensualDataZone } = useZonaArticuloMensual(activeZone);
  // Cluster support
  const [activeCluster, setActiveCluster] = useState<ZonaCluster | null>(null);
  const [showClusterMgr, setShowClusterMgr] = useState(false);
  const { data: clusterArts } = useOutboundClusterCompaniaResumen(activeCluster?.zonas ?? []);
  ...
  const getArtPromedios = useCallback((idCompania: string, articulo: string) => {
    ...
    let totalMov = 0;
    let totalUnid = 0;
    for (const m of Object.values(mesData)) {
      totalMov += m.movimientos;
      totalUnid += m.unidades;
    }
    return {
      promMov: meses > 0 ? Math.round(totalMov / meses) : 0,
      promUnid: meses > 0 ? Math.round(totalUnid / meses) : 0,
      mesesActivos: meses,
    };
  }, [articuloMensualMap]);
  ...
  const [celdasFormulas, setCeldasFormulas] = useState<Record<string, ZonaCeldaFormula[]>>({});
  ...
  // ── Column reorder state (all columns) ─────────────────────────────
  ...
  const loadZonaColumnas = useCallback(async (zona: string) => {
    setColLoading(true);
    setColOrder([]);
    const { data: cols } = await supabase.from('costos_outbound_zona_columnas').select('*').eq('zona', zona).order('orden');
    const colArray = (cols ?? []) as ZonaColumnaDinamica[];
    setZonaColumnas(colArray);
    if (colArray.length > 0) {
      const colIds = colArray.map(c => c.id);
      const { data: cells } = await supabase.from('costos_outbound_zona_celdas').select('*').in('columna_id', colIds);
      const byCol: Record<string, ZonaCeldaFormula[]> = {};
      colIds.forEach(id => { byCol[id] = []; });
      (cells ?? []).forEach(c => {
        const cell = c as ZonaCeldaFormula;
        if (byCol[cell.columna_id]) byCol[cell.columna_id].push(cell);
      });
      setCeldasFormulas(byCol);
    } else {
      setCeldasFormulas({});
    }
    const savedKey = `costos-outbound-zone-order-${zona}`;
    try {
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    localStorage.setItem(`costos-outbound-zone-order-${activeZone}`, JSON.stringify(reordered));
    // Persist dynamic column order to DB
    ...
    for (let i = 0; i < dynCols.length; i++) {
      ...
      if (col && col.orden !== i) {
        await supabase.from('costos_outbound_zona_columnas').update({ orden: i }).eq('id', dynCols[i]);
      }
    }
  ...
  const handleAddColumn = useCallback(async () => {
    ...
    const { data: newCol } = await supabase.from('costos_outbound_zona_columnas').insert({ zona: activeZone, nombre: name, tipo: 'formula', orden }).select().maybeSingle();
    if (newCol) {
  ...
  const handleDeleteColumn = useCallback(async (colId: string) => {
    ...
    await supabase.from('costos_outbound_zona_celdas').delete().eq('columna_id', colId);
    await supabase.from('costos_outbound_zona_columnas').delete().eq('id', colId);
  ...
  const handleRenameColumn = useCallback(async (colId: string, nombre: string) => {
    ...
    await supabase.from('costos_outbound_zona_columnas').update({ nombre }).eq('id', colId);
  }, []);
  ...
  const handleSaveColumnFormula = useCallback(async (formula: string) => {
    ...
    await supabase.from('costos_outbound_zona_columnas').update({ formula: formula || null }).eq('id', columnaId);
    setEditingColumnFormula(null);
  }, [editingColumnFormula]);

  // Zone totals computed early so buildRowVarMap can reference them
  ...
  // ── Formula engine: full system variable defs and varMap ──────────────
  // Exponemos TODAS las variables del motor (inversiones, gastos, MO, volúmenes,
  // distribución, factores, COSTOS_TOTAL_*, etc.) para usar en fórmulas de zona.
  const systemVarDefs = useMemo<VariableDef[]>(() => {
    try {
      const data = toAllDataSources(formulaCtx);
      const allDefs = buildVariableDefs(data);
      return allDefs.filter(d => !d.token.startsWith('SUM_COSTOS_'));
    } catch {
      return [];
    }
  }, [formulaCtx]);
  ...
  // ── Precompute sum of all article averages for distribution bars ──
  // MUST be defined BEFORE buildRowVarMap since it references them
  const sumAllPromMovArts = useMemo(() => {
    let sum = 0;
    for (const art of activeZoneArtsAll) {
      sum += getArtPromedios(art.idCompania, art.articulo).promMov;
    }
    return sum;
  }, [activeZoneArtsAll, getArtPromedios]);

  const sumAllPromUnidArts = useMemo(() => {
    let sum = 0;
    for (const art of activeZoneArtsAll) {
      sum += getArtPromedios(art.idCompania, art.articulo).promUnid;
    }
    return sum;
  }, [activeZoneArtsAll, getArtPromedios]);

  // ── Build varMap for an article row ──
  // Keys are WITHOUT braces because resolveTokens strips braces before lookup
  const buildRowVarMap = useCallback((art: ZonaArticuloDetalleRow): Record<string, number> => {
    ...
    const artVars: Record<string, number> = {
      'MOV': art.movimientos,
      'UNID': art.unidades,
      'ZONA_MOV': zoneTotalMov,
      'ZONA_UNID': zoneTotalUnid,
      ...
      'PCT_PROM_MOV_MES': pctPromMov,
      'PCT_PROM_UNID_MES': pctPromUnid,
    };
    // Merge: system vars first, article vars override (article vars take priority)
    return { ...systemVarMap, ...artVars };
  }, [zoneTotalMov, zoneTotalUnid, systemVarMap, getArtPromedios, sumAllPromMovArts, sumAllPromUnidArts]);

  // ── Precompute all dynamic cell values for the current zone ──────────
  // Uses column-level formula applied to each article row
  const computedCells = useMemo(() => {
    ...
    for (const col of zonaColumnas) {
      ...
      // Only MOV / UNID / PCT_MOV / PCT_UNID are per-article vars
      // ZONA_MOV / ZONA_UNID are zone-level constants → global formula
  ...
  // ── Filtered & sorted articles (before footerTotals to avoid TDZ) ───
  ...
  const getSortValue = useCallback((art: ZonaArticuloDetalleRow, colKey: string): number | string => {
  ...
  const footerTotals = useMemo(() => {
    ...
    for (const col of zonaColumnas) {
      ...
      if (isGlobal && sortedArtsAll.length > 0) {
        // Global formula: show the single value, don't multiply by article count
        const cell = computedCells[col.id]?.[sortedArtsAll[0].articulo];
        result[col.id] = cell?.value ?? 0;
      } else {
        // Per-article formula: sum all values
  ...
  if (loading) return <div className="flex items-center justify-center py-16"><div className="flex flex-col items-center gap-3"><div className="w-8 h-8 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" /><p className="text-xs text-slate-400">Calculando resumen por zona...</p></div></div>;
  if (rows.length === 0) return <div className="flex items-center justify-center py-16"><p className="text-sm text-slate-400">No hay datos disponibles.</p></div>;
  ...
  // Zone filtered totals for search results
  const zoneTotalMovPct = zoneTotalMov;
  const zoneTotalUnidPct = zoneTotalUnid;
  ...
  return (
    <div className="space-y-5">
      ...
      {showClusterMgr && <ZonaClusterManager tableName="costos_outbound_clusters" clusters={clusters} zonas={rows.map(r => r.zona)} onChanged={onClustersChange} />}
      ...
      <div className="flex gap-2 flex-wrap">
        {clusters.map(cluster => {
          ...
          return (
            <button key={cluster.id} onClick={() => { setActiveCluster(cluster); setActiveZone(''); setPrevZone(''); }}
        ...
        {rows.map((row, i) => {
          ...
          const zoneArtsCount = row.articulos_distintos ?? 0;
          return (
            <button
              key={row.zona}
              onClick={() => { setActiveCluster(null); switchZone(row.zona); }}
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col lg:flex-row lg:items-start gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-800">
                  <i className="ri-map-pin-line text-lg text-white" />
                </div>
                <div>
                  <h4 className="text-base font-bold text-slate-800">{activeZone}</h4>
                  <p className="text-xs text-slate-400">
                    {fmtPct(pctGlobalMov)}% del total global de movimientos · {fmtPct(pctGlobalUnid)}% de unidades
                  </p>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 flex-shrink-0">
              <div className="bg-emerald-50 rounded-lg px-4 py-2.5 border border-emerald-100 text-center min-w-[90px]">
                <p className="text-[10px] text-emerald-600 font-medium uppercase tracking-wider">Movimientos</p>
                <p className="text-base font-bold text-emerald-700 mt-0.5">{fmt(zoneTotalMov)}</p>
              </div>
              <div className="bg-rose-50 rounded-lg px-4 py-2.5 border border-rose-100 text-center min-w-[90px]">
                <p className="text-[10px] text-rose-600 font-medium uppercase tracking-wider">Unidades</p>
                <p className="text-base font-bold text-rose-700 mt-0.5">{fmt(zoneTotalUnid)}</p>
              </div>
              ...
              <div className="bg-amber-50 rounded-lg px-4 py-2.5 border border-amber-100 text-center min-w-[90px]">
                <p className="text-[10px] text-amber-600 font-medium uppercase tracking-wider">Prom. Unid/Art</p>
                <p className="text-base font-bold text-amber-700 mt-0.5">{fmtDec(activeZoneArtsAll.length > 0 ? zoneTotalUnid / activeZoneArtsAll.length : 0)}</p>
              </div>
              <div className="bg-teal-50 rounded-lg px-4 py-2.5 border border-teal-100 text-center min-w-[90px]">
                <p className="text-[10px] text-teal-600 font-medium uppercase tracking-wider">Prom. Mov/Mes</p>
                <p className="text-base font-bold text-teal-700 mt-0.5">{fmtDec(activeZoneData?.prom_movimientos_mes ?? 0)}</p>
              </div>
              <div className="bg-violet-50 rounded-lg px-4 py-2.5 border border-violet-100 text-center min-w-[90px]">
                <p className="text-[10px] text-violet-600 font-medium uppercase tracking-wider">Prom. Unid/Mes</p>
                <p className="text-base font-bold text-violet-700 mt-0.5">{fmtDec(activeZoneData?.prom_unidades_mes ?? 0)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                type="text"
                placeholder={`Buscar artículo en ${activeZone}...`}
                value={search}
                onChange={e => setSearch(e.target.value)}
                className="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-100 focus:border-emerald-300 outline-none bg-white text-slate-700 placeholder:text-slate-400"
              />
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <select
                ...
                className="px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-100 focus:border-emerald-300 outline-none bg-white text-slate-700 min-w-[160px]"
              >
                <option value="all">Todas las compañías</option>
                {companiasUnicas.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <ExportMenu
                filenameBase={`zona-${activeZone}-articulos`}
                getExportData={() => {
                  ...
                  const exportRows = sortedArtsAll.map(art => {
                    ...
                    return [
                      ...
                      artPctMov.toFixed(2),
                      art.unidades,
                      artPctUnid.toFixed(2),
                      proms.promMov.toFixed(2),
                      proms.promUnid.toFixed(2),
                      (sumAllPromMovArts > 0 ? (proms.promMov / sumAllPromMovArts) * 100 : 0).toFixed(2),
                      (sumAllPromUnidArts > 0 ? (proms.promUnid / sumAllPromUnidArts) * 100 : 0).toFixed(2),
                      ...mesesDisponibles.flatMap(m => {
                        const md = articuloMensualMap[key]?.[m.mes];
                        return [md ? String(md.movimientos) : '', md ? String(md.unidades) : ''];
                      }),
          ...
          {/* Zone switching loading overlay */}
          {(zoneSwitching || isPending) && (
            <div className="relative">
              <div className="absolute inset-0 z-20 bg-white/80 backdrop-blur-sm rounded-lg flex items-center justify-center" style={{ minHeight: 200 }}>
                <div className="flex flex-col items-center gap-3">
                  <div className="w-10 h-10 border-2 border-amber-500 border-t-transparent rounded-full animate-spin" />
                  <p className="text-sm font-semibold text-slate-700">Cargando <strong className="text-amber-600">{activeZone}</strong>...</p>
                  <p className="text-xs text-slate-400">Preparando fórmulas y columnas dinámicas</p>
                </div>
              </div>
            </div>
          )}
          {/* Small inline loading for column-only changes */}
          {colLoading && !zoneSwitching && !isPending && (
            <div className="flex items-center gap-3 px-4 py-2.5 bg-amber-50 border border-amber-200 rounded-lg mb-2">
              <div className="w-5 h-5 border-2 border-amber-500 border-t-transparent rounded-full animate-spin flex-shrink-0" />
              <span className="text-sm font-medium text-amber-700">Cargando columnas de <strong>{activeZone}</strong>...</span>
            </div>
          )}
                    ...
                    {colLoading && zonaColumnas.length === 0 && (
                      <th className="px-3 py-2.5 bg-slate-50 border-r border-slate-100">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="w-4 h-4 border-2 border-amber-400 border-t-transparent rounded-full animate-spin ml-1" />
                        </div>
                      </th>
                    )}
                  </SortableContext>
                  {/* Add column button / Loading indicator */}
                    ...
                    {colLoading ? (
                      ...
                      <div className="flex items-center gap-1 px-1">
                        <input
                          ...
                          placeholder="Nombre..."
                          className="w-[120px] px-2 py-1 text-xs border border-amber-300 rounded-md focus:outline-none focus:ring-1 focus:ring-amber-400 bg-white"
                          autoFocus
                        />
                        <button onClick={handleAddColumn} disabled={!newColName.trim()} className="w-6 h-6 flex items-center justify-center rounded-md bg-amber-500 hover:bg-amber-600 text-white cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                      ...
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center rounded-lg border-2 border-dashed border-slate-300 text-slate-400 hover:border-amber-400 hover:text-amber-500 hover:bg-amber-50 cursor-pointer transition-all"
                ...
                {paginatedArts.length === 0 ? (
                  <tr>
                    <td colSpan={columnOrder.length + 1} className="px-3 py-10 text-center text-slate-400">
                      {search ? 'Sin resultados para esta búsqueda' : 'Sin artículos en esta zona'}
                    </td>
                  </tr>
                ) : (
                  paginatedArts.map((art, ai) => {
                    ...
                    const pctPromUnid = sumAllPromUnidArts > 0 ? (proms.promUnid / sumAllPromUnidArts) * 100 : 0;
                    return (
                      <tr key={`${activeZone}-${art.idCompania}-${art.articulo}`} className={`border-t border-slate-100 hover:bg-amber-50/50 ${ai % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                        {columnOrder.map(colKey => {
                          if (colKey.startsWith('FIXED:')) {
                            const key = colKey.slice(6);
                            switch (key) {
                            ...
                            return (
                              <td key={col.id} onClick={(e) => handleOpenColumnFormulaEditor(col, e)} className={`px-3 py-2 text-right border-r border-slate-100 cursor-pointer transition-colors ${hasFormula ? 'hover:bg-amber-100/60' : 'hover:bg-slate-100'}`} title={hasFormula ? cell?.formula : 'Clic para agregar fórmula'}>
                                {hasFormula ? (
                                  cell?.error ? (
                                    ...
                                    <span className="text-amber-700 font-medium tabular-nums">{new Intl.NumberFormat('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(cell.value)}</span>
                                  ) : (
          ...
          {/* Formula editor popover for column formula */}
          {editingColumnFormula && (() => {
            // Use the first article's varMap as preview since column formula uses same expression for all rows
...
// ── Sortable Dynamic Column Header (drag handle + column formula) ─────────
...
function SortableColHeader({ col, onDelete, onEditFormula, onRename, onSort, sortIconClass }: { col: ZonaColumnaDinamica; onDelete: (id: string) => void; onEditFormula: (col: ZonaColumnaDinamica, e: React.MouseEvent) => void; onRename: (id: string, nombre: string) => void; onSort: () => void; sortIconClass: string }) {
  ...
  return (
    <th ref={setNodeRef} style={style} className={`px-2 py-2.5 border-r font-semibold ${hasFormula ? 'bg-amber-100/60 border-amber-200' : 'bg-amber-50/50 border-amber-100'}`}>
      <div className="flex items-center gap-1.5">
        {/* Drag handle */}
        <button
          {...attributes}
        ...
        <div className="flex items-center gap-1 min-w-0 flex-1">
          {editing ? (
            <input
              ...
              className="text-xs text-amber-700 bg-white border border-amber-300 rounded px-1.5 py-0.5 w-full min-w-[80px] focus:outline-none focus:ring-1 focus:ring-amber-400"
              autoFocus
            />
          ) : (
            <div className="flex items-center gap-0.5 min-w-0 group/name">
              <span onClick={onSort} className="text-xs text-amber-700 whitespace-nowrap max-w-[120px] overflow-hidden text-ellipsis cursor-pointer hover:underline">{col.nombre}</span>
              ...
              <button
                ...
                className="w-4 h-4 flex items-center justify-center rounded text-slate-300 hover:text-amber-500 hover:bg-amber-100 cursor-pointer flex-shrink-0 opacity-0 group-hover/name:opacity-100 transition-opacity"
          ...
          {hasFormula && (
            <span className="text-[10px] px-1 py-0.5 rounded bg-amber-200 text-amber-700 font-mono font-bold flex-shrink-0">fx</span>
          )}
        </div>
        {/* Edit formula button */}
        <button
          onClick={(e) => onEditFormula(col, e)}
          className={`w-5 h-5 flex items-center justify-center rounded cursor-pointer flex-shrink-0 transition-colors ${hasFormula ? 'text-amber-600 hover:text-amber-800 hover:bg-amber-200' : 'text-slate-400 hover:text-amber-500 hover:bg-amber-100'}`}
