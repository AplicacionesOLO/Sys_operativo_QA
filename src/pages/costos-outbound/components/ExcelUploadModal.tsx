...
import { parseVolumenesMasivoExcel } from '@/lib/volumenesMasivoExcelParser';
import type { MasivoParseResult } from '@/lib/volumenesMasivoExcelParser';
...
export default function ExcelUploadModal({ onClose, onSuccess }: ExcelUploadModalProps) {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    const result = parseVolumenesMasivoExcel(buffer);
    setParsed(result);
    setStep('preview');
  }, []);
  ...
  const handleUpload = async () => {
    ...
    setUploadError('');
    try {
      // Carga masiva acumulativa — mantiene datos históricos previos
      // Zonas excluidas permanentemente y mapeos de unificación
      const ZONAS_EXCLUIDAS = ['ZP37', 'ZP23'];
      const UNIFICACION_ZONAS: Record<string, string> = {
        'ZP18': 'ZP15',
        'ZP32': 'ZP15',
        'ZP44': 'ZP15',
        'ZP28': 'ZP15',
      ...
      for (let i = 0; i < parsed.batches.length; i++) {
        ...
        const filtrados = batch.filter(row => {
          let zona = row['Zona Picking'];
          if (!zona) return true;
          const zonaStr = String(zona);
          if (ZONAS_EXCLUIDAS.includes(zonaStr)) return false;
          // Unificar zonas
          if (UNIFICACION_ZONAS[zonaStr]) {
            row['Zona Picking'] = UNIFICACION_ZONAS[zonaStr];
          }
          return true;
        });
        totalExcluidos += batch.length - filtrados.length;
        if (filtrados.length > 0) {
          const { error } = await supabase.from('volumenes_raw').insert(
            filtrados.map(rawData => ({ raw_data: rawData }))
          );
          if (error) throw new Error(`Error en lote ${i + 1} de ${parsed.batches.length}: ${error.message}`);
          totalInsertados += filtrados.length;
        }
      }
      
      // Update parsed with actual inserted count for the done screen
      if (totalExcluidos > 0) {
        setParsed(prev => prev ? { ...prev, totalRows: totalInsertados } : prev);
      }
          ...
          {step === 'select' && (
            ...
            >
              <div className="w-14 h-14 flex items-center justify-center rounded-full bg-amber-50">
                <i className="ri-database-2-line text-3xl text-amber-500" />
              </div>
            ...
            <div className="space-y-4">
              {fileName && (
                <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-lg">
                  <div className="w-5 h-5 flex items-center justify-center text-emerald-500"><i className="ri-file-excel-2-line" /></div>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-3 text-center">
                      <p className="text-xl font-bold text-emerald-700">{parsed.headers.length}</p>
                      <p className="text-xs text-emerald-600 mt-0.5">Columnas</p>
                    </div>
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 flex items-start gap-3">
                    ...
                    <p className="text-xs text-amber-700"><strong>Importante:</strong> Esta carga <strong>reemplazará todos los datos masivos anteriores</strong>. Se subirán {parsed.batches.length} lotes de hasta 1.000 filas cada uno. Las zonas <strong>ZP37 y ZP23 serán excluidas</strong>. ZP18, ZP32, ZP44, ZP28 y ZP20 se unifican con ZP15, y ZP34 con ZP19 automáticamente.</p>
                  </div>
                </div>
              )}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between gap-3 flex-shrink-0">
          ...
          {step === 'select' && (
            <>
              ...
              <button onClick={() => inputRef.current?.click()} className="px-5 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">Seleccionar archivo</button>
            </>
          )}
          {step === 'preview' && (
            <>
              ...
              <button disabled={hasErrors} onClick={handleUpload} className="px-5 py-2 bg-amber-500 hover:bg-amber-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">Confirmar y subir</button>
            </>
          )}
          {step === 'done' && (
            <button onClick={() => { onSuccess(); onClose(); }} className="ml-auto px-5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">Ver datos</button>
          )}
        </div>
      </div>
    </div>
