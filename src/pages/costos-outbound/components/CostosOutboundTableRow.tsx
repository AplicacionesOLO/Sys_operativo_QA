...
import type { CostoOutboundColumna, CostoOutboundFila, FormulaConfig } from '@/types/costos_outbound';
import { formatCellValue } from '@/types/costos_outbound';
...
interface CostosOutboundTableRowProps {
  fila: CostoOutboundFila;
  columnas: CostoOutboundColumna[];
...
export default function CostosOutboundTableRow({
  fila, columnas, onUpdate, onUpdateCell, onDelete,
  onSaveRowFormula, onClearRowFormula,
  saving, formulaCtx,
}: CostosOutboundTableRowProps) {
  const [editing, setEditing] = useState<EditingCell | null>(null);
  const [hovered, setHovered] = useState(false);
  const [formulaModalCol, setFormulaModalCol] = useState<CostoOutboundColumna | null>(null);
  ...
  const renderFormulaCell = (col: CostoOutboundColumna) => {
    ...
    return (
      <div className="flex items-center gap-1.5 group/formula">
        <div className="flex items-center gap-1.5 flex-1 min-w-0">
          {rowFormula ? <span className="text-xs px-1 py-0.5 rounded bg-emerald-100 text-emerald-600 font-mono font-semibold flex-shrink-0">fx</span> : <span className="text-xs px-1 py-0.5 rounded bg-violet-100 text-violet-600 font-mono font-semibold flex-shrink-0">fx</span>}
  ...
  const renderDynamicCell = (columnaId: string, col: CostoOutboundColumna) => {
    ...
    if (isEditingThis) {
      if (col.tipo === 'select') {
        return (
          <select ref={inputRef as React.RefObject<HTMLSelectElement>} value={editing!.value} onChange={e => setEditing({ field: columnaId, value: e.target.value })} onBlur={commitEdit} className="w-full bg-white border border-emerald-400 rounded px-2 py-1 text-xs focus:outline-none">
      ...
      <tr onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="group transition-colors hover:bg-slate-50 border-t border-slate-100">
        <td className={`${cellClass} w-36`}>
          {editing?.field === 'proceso' ? (
            <input ref={inputRef as React.RefObject<HTMLInputElement>} type="text" value={editing.value} onChange={e => setEditing({ field: 'proceso', value: e.target.value })} onBlur={commitEdit} onKeyDown={handleKeyDown} className="w-full bg-white border border-emerald-400 rounded px-2 py-1 text-sm font-medium focus:outline-none" />
          ) : (
            <span className="block truncate cursor-text font-semibold text-slate-700 hover:text-emerald-600 transition-colors text-sm" onClick={() => startEdit('proceso', fila.proceso)} title={fila.proceso}>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex-1 min-w-0">
              {editing?.field === 'subproceso' ? (
                <input ref={inputRef as React.RefObject<HTMLInputElement>} type="text" value={editing.value} onChange={e => setEditing({ field: 'subproceso', value: e.target.value })} onBlur={commitEdit} onKeyDown={handleKeyDown} className="w-full bg-white border border-emerald-400 rounded px-2 py-1 text-sm focus:outline-none" />
              ) : (
                <span className={`block truncate cursor-text text-sm font-medium transition-colors ${fila.subproceso ? 'text-slate-700 hover:text-emerald-600' : 'text-slate-300 italic font-normal'}`} onClick={() => startEdit('subproceso', fila.subproceso)} title={fila.subproceso}>
