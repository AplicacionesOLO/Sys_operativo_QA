...
import CostosOutboundTableRow from './CostosOutboundTableRow';

interface CostosOutboundTableProps {
  columnas: CostoOutboundColumna[];
  filas: CostoOutboundFila[];
  savingId: string | null;
  onAddColumn: () => void;
  onEditColumn: (col: CostoOutboundColumna) => void;
...
export default function CostosOutboundTable({
    ...
    <div className="bg-white rounded-xl border border-slate-200">
          ...
          <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
            <tr>
              ...
              <th className="px-3 py-3 bg-slate-800" style={{ width: 48, minWidth: 48 }}>
                <button onClick={onAddColumn} className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-700 hover:bg-amber-500 text-slate-300 hover:text-white transition-all cursor-pointer" title="Agregar columna">
                ...
                <td colSpan={columnas.length + 3} className="px-8 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-400 text-xs mt-1">Agrega una fila para comenzar a registrar costos outbound</p>
                    </div>
                    <button onClick={onAddFila} className="mt-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">
      ...
      {hasData && (
        <div className="px-4 py-3 border-t border-slate-100 flex items-center justify-between gap-2 flex-wrap">
          <button onClick={onAddFila} className="flex items-center gap-2 text-sm text-slate-500 hover:text-amber-600 hover:bg-amber-50 px-3 py-1.5 rounded-lg transition-all cursor-pointer whitespace-nowrap">
            <div className="w-4 h-4 flex items-center justify-center"><i className="ri-add-line" /></div>Agregar fila
          </button>
          <ExportMenu
            filenameBase="costos-outbound-operacion"
            getExportData={() => {
