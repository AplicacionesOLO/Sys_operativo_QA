...
interface InboundZonaCeldaFormulaEditorProps {
  formula: string;
    ...
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 flex-shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 flex items-center justify-center rounded-md bg-emerald-100">
            <i className="ri-functions text-xs text-emerald-600" />
          </div>
      ...
      <div className="px-4 py-2 border-b border-slate-100 flex-shrink-0">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-emerald-300 focus:border-emerald-300 outline-none bg-white text-slate-700 placeholder:text-slate-400"
          />
          {search && (
              ...
              {filteredArticuloTokens.map(t => {
                ...
                return (
                  <button
                    key={t.token}
                    onClick={() => insertToken(t.token)}
                    className="px-2 py-1 text-xs rounded-md border border-emerald-200 bg-emerald-50 text-emerald-700 hover:border-emerald-400 hover:bg-emerald-100 cursor-pointer transition-colors whitespace-nowrap flex items-center gap-1"
                    title={t.desc}
                  >
                    <span className="font-mono font-medium">{t.token}</span>
                    ...
                    <span className="text-emerald-500">· {t.label}</span>
                  </button>
                );
              })}
            </div>
                  ...
                  {g.defs.map(d => {
                    ...
                    return (
                      <button
                        ...
                        className="px-2 py-1 text-xs rounded-md border border-slate-200 bg-slate-50 text-slate-600 hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700 cursor-pointer transition-colors whitespace-nowrap flex items-center gap-1"
                        title={d.description}
                      >
      ...
      <div className="px-4 py-3 border-t border-slate-200 bg-slate-50/50 flex-shrink-0 space-y-2">
        <div className={`rounded-lg border-2 transition-colors ${validationError && expr.trim() ? 'border-rose-300' : 'border-slate-200 focus-within:border-emerald-400'}`}>
          <textarea
            ref={textareaRef}
            value={expr}
        ...
        <p className="text-[11px] text-slate-400">
          <i className="ri-lightbulb-line mr-1 text-emerald-400" />
      ...
      <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-slate-200 bg-slate-50 rounded-b-xl flex-shrink-0">
        ...
        <button
          ...
          className="px-4 py-1.5 text-xs font-medium bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg cursor-pointer transition-colors whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Guardar fórmula
        </button>
      </div>
