...
const ARTICULO_TOKENS: { token: string; label: string; desc: string }[] = [
  { token: '{CANTIDAD}', label: 'Unidades', desc: 'Unidades del artículo en esta categoría' },
  { token: '{ZONA_CANTIDAD}', label: 'Total Unid. Categoría', desc: 'Total de unidades de toda la categoría' },
  { token: '{PCT_CANTIDAD}', label: '% Unid. del artículo', desc: 'Porcentaje de unidades del artículo / total categoría * 100' },
  { token: '{CANT_UBICACIONES}', label: 'Cant. Ubicaciones', desc: 'Cantidad de ubicaciones donde aparece este artículo' },
  { token: '{ZONA_UBICACIONES}', label: 'Total Ubic. Categoría', desc: 'Total de ubicaciones de toda la categoría' },
];
...
export default function ZonaCeldaFormulaEditor({ formula, varMap, onSave, onCancel, position, systemVarDefs, systemVarMap }: ZonaCeldaFormulaEditorProps) {
  ...
  const mergedVarMap = useMemo(() => ({ ...systemVarMap, ...varMap }), [systemVarMap, varMap]);

  const validationError = expr.trim() ? validateExpression(expr) : null;
  const result = !validationError && expr.trim() ? evalFormula(expr, mergedVarMap) : null;
    ...
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 flex-shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 flex items-center justify-center rounded-md bg-teal-100">
            <i className="ri-functions text-xs text-teal-600" />
          </div>
      ...
      <div className="px-4 py-2 border-b border-slate-100 flex-shrink-0">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-teal-300 focus:border-teal-300 outline-none bg-white text-slate-700 placeholder:text-slate-400"
          />
          {search && (
              ...
              {filteredArticuloTokens.map(t => {
                ...
                return (
                  <button
                    ...
                    className="px-2 py-1 text-xs rounded-md border border-teal-200 bg-teal-50 text-teal-700 hover:border-teal-400 hover:bg-teal-100 cursor-pointer transition-colors whitespace-nowrap flex items-center gap-1"
                    title={t.desc}
                  >
                    <span className="font-mono font-medium">{t.token}</span>
                    {val !== undefined && val !== null && (
                      <span className="text-teal-600 text-[10px] font-semibold tabular-nums">
                  ...
                  {g.defs.map(d => {
                    ...
                    return (
                      <button
                        ...
                        className="px-2 py-1 text-xs rounded-md border border-slate-200 bg-slate-50 text-slate-600 hover:border-teal-300 hover:bg-teal-50 hover:text-teal-700 cursor-pointer transition-colors whitespace-nowrap flex items-center gap-1"
                        title={d.description}
                      >
      ...
      <div className="px-4 py-3 border-t border-slate-200 bg-slate-50/50 flex-shrink-0 space-y-2">
        <div className={`rounded-lg border-2 transition-colors ${validationError && expr.trim() ? 'border-rose-300' : 'border-slate-200 focus-within:border-teal-400'}`}>
          <textarea
            ref={textareaRef}
            value={expr}
            onChange={e => setExpr(e.target.value)}
            onKeyDown={handleKeyDown}
            placeholder='Ej: {CANTIDAD} * 1500 / {ZONA_CANTIDAD}'
            rows={3}
        ...
        {!validationError && expr.trim() && result && (
          <div className={`px-3 py-2 rounded-lg text-xs ${result.ok ? 'bg-teal-50 border border-teal-200' : 'bg-rose-50 border border-rose-200'}`}>
            {result.ok ? (
              <div className="flex items-center gap-2">
                <i className="ri-checkbox-circle-line text-teal-500" />
                <span className="text-teal-700 font-medium">Resultado:</span>
                <span className="text-teal-800 font-bold tabular-nums">
        ...
        <p className="text-[11px] text-slate-400">
          <i className="ri-lightbulb-line mr-1 text-teal-400" />
      ...
      <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-slate-200 bg-slate-50 rounded-b-xl flex-shrink-0">
        ...
        <button
          ...
          className="px-4 py-1.5 text-xs font-medium bg-teal-500 hover:bg-teal-600 text-white rounded-lg cursor-pointer transition-colors whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Guardar fórmula
        </button>
      </div>
