...
import { parseAlmacenMasivoExcel } from '@/lib/almacenMasivoExcelParser';
import type { MasivoParseResult } from '@/lib/almacenMasivoExcelParser';
...
export default function ExcelUploadModal({ onClose, onSuccess }: ExcelUploadModalProps) {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    const result = parseAlmacenMasivoExcel(buffer);
    setParsed(result);
    setStep('preview');
  }, []);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < parsed.batches.length; i++) {
        ...
        if (batch.length > 0) {
          const { error } = await supabase.from('costos_almacen_raw').insert(
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-4 max-h-[90vh] flex flex-col overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-base font-semibold text-slate-800">Cargar datos masivos - Almacén</h2>
            <p className="text-xs text-slate-400 mt-0.5">Carga el archivo Excel con los datos de almacenaje. Todas las columnas, sin límite de filas.</p>
          </div>
        ...
        <div className="px-6 py-5 overflow-y-auto flex-1">
              ...
              {!hasErrors && parsed && (
                <div className="space-y-3">
                  ...
                  <div className="bg-teal-50 border border-teal-200 rounded-lg px-4 py-3 flex items-start gap-3">
                    <div className="w-5 h-5 flex items-center justify-center text-teal-500 mt-0.5 flex-shrink-0"><i className="ri-information-line" /></div>
                    <p className="text-xs text-teal-700">Esta carga es <strong>acumulativa</strong> — mantiene los datos históricos previos. Se subirán {parsed.batches.length} lotes de hasta 1.000 filas cada uno a la tabla de almacén.</p>
                  </div>
                </div>
              )}
          ...
          {step === 'uploading' && (
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <p className="text-sm text-slate-600">Guardando datos en almacén...</p>
          ...
          {step === 'done' && (
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">{parsed?.totalRows.toLocaleString('es-CO') ?? 0} filas guardadas en almacén.</p>
              </div>
            </div>
          )}
        </div>
