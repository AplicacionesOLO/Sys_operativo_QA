...
import type { CostoAlmacenColumna, ColumnType, FormulaConfig } from '@/types/almacen';
import { COLUMN_TYPES } from '@/types/almacen';
...
interface AddColumnModalProps {
  ...
  editing?: CostoAlmacenColumna | null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className={`bg-white rounded-xl shadow-xl w-full ${modalWidth} overflow-hidden flex flex-col max-h-[92vh] transition-all duration-200`}>
          ...
          <div className="space-y-5">
            <div className={`grid gap-5 ${isFormulaType ? 'grid-cols-2' : 'grid-cols-1'}`}>
              <div>
                ...
                <input type="text" value={nombre} onChange={e => { setNombre(e.target.value); setError(''); }} placeholder="ej. Costo total, Tarifa..." className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-700 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400" />
                  ...
                  {COLUMN_TYPES.map(ct => (
                    <button key={ct.value} onClick={() => { setTipo(ct.value); setError(''); }}
                      className={`flex items-center gap-1.5 px-2.5 py-2 rounded-lg border text-xs font-medium transition-all cursor-pointer whitespace-nowrap ${
                        tipo === ct.value ? ct.value === 'formula' ? 'border-violet-500 bg-violet-50 text-violet-700' : 'border-teal-500 bg-teal-50 text-teal-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50'
                      }`}
                    >
            ...
            {tipo === 'select' && (
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" value={newOpcion} onChange={e => setNewOpcion(e.target.value)} onKeyDown={e => e.key === 'Enter' && addOpcion()} placeholder="Nueva opción..." className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400" />
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-end gap-3 flex-shrink-0">
          ...
          <button onClick={handleSave} className={`px-5 py-2 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap ${isFormulaType ? 'bg-violet-500 hover:bg-violet-600' : 'bg-teal-500 hover:bg-teal-600'}`}>
