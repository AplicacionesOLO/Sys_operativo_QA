...
import InboundZonaCeldaFormulaEditor from './components/ZonaCeldaFormulaEditor';
...
export default function CostosInboundPage() {
  const [loading, setLoading] = useState(true);
  const [showUpload, setShowUpload] = useState(false);
  const [formulaCtx, setFormulaCtx] = useState<FormulaContext>(EMPTY_FORMULA_CTX);

  const { data: masivoData, load: loadMasivo } = useInboundMasivoResumen();
  const { data: resumenCompleto, loading: resumenLoading, load: loadResumen } = useInboundArticuloResumen();
  ...
  if (loading) {
    return (
      <AppLayout title="Costos Inbound" subtitle="Cargando módulo...">
  ...
  return (
    <AppLayout
      title="Costos Inbound"
      subtitle="Datos masivos de inbound + resumen por zona y artículo"
      actions={
        <div className="flex items-center gap-2">
          <button onClick={() => setShowUpload(true)} className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-semibold text-slate-800">Datos masivos de Inbound</h3>
              <p className="text-xs text-slate-400 mt-0.5">Archivo cargado tal cual, sin transformaciones</p>
            </div>
            {hasMasivo && (
              <span className="text-xs px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 font-medium">{fmt(masivoData!.totalRegistros)} registros</span>
            )}
          </div>
          ...
          {!hasMasivo ? (
            <div className="px-6 py-12 flex flex-col items-center gap-4">
              <div className="w-14 h-14 flex items-center justify-center rounded-full bg-emerald-50">
                <i className="ri-database-2-line text-2xl text-emerald-400" />
              </div>
              <div className="text-center max-w-sm">
                <p className="text-slate-700 font-semibold text-sm">Sin datos masivos</p>
                <p className="text-slate-400 text-xs mt-1">Carga un archivo Excel con los datos de inbound para ver el resumen.</p>
              </div>
              <button onClick={() => setShowUpload(true)} className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">
              ...
              {tab === 'operacion' && (
                <div className="py-16 flex flex-col items-center gap-4">
                  ...
                  <div className="text-center max-w-sm">
                    ...
                    <p className="text-slate-400 text-xs mt-1">La matriz de costos con fórmulas para inbound estará disponible pronto.</p>
                  </div>
                </div>
              )}
            </div>
...
function UploadModalWrapper({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  const InboundExcelUploadModal = React.lazy(() => import('./components/ExcelUploadModal'));
  return (
    <React.Suspense fallback={null}>
      <InboundExcelUploadModal onClose={onClose} onSuccess={onSuccess} />
    </React.Suspense>
  );
}
...
function InboundResumenTab({ data }: { data: { totalRegistros: number; headers: string[] } }) {
  const fmt = (n: number) => new Intl.NumberFormat('es-CO', { maximumFractionDigits: 0 }).format(n);
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard icon="ri-database-2-line" iconColor="text-emerald-500" bg="bg-emerald-50" label="Total registros" value={fmt(data.totalRegistros)} sub="filas cargadas" />
...
function InboundArticuloResumenTable({ data, loading, globalTotals }: { data?: ArticuloResumenRow[]; loading: boolean; globalTotals?: { totalMov: number; totalUnid: number; totalCount: number } }) {
  const fmt = (n: number) => new Intl.NumberFormat('es-CO', { maximumFractionDigits: 0 }).format(n);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <ExportMenu
          filenameBase="inbound-resumen-articulos"
          getExportData={() => {
...
function InboundZonaResumenTable({ data, loading, globalTotals, formulaCtx, clusters, onClustersChange }: { data?: ZonaResumenRow[]; loading: boolean; globalTotals?: { totalMov: number; totalUnid: number; totalCount: number }; formulaCtx: FormulaContext; clusters: ZonaCluster[]; onClustersChange: () => void }) {
  ...
  const [zonaColumnas, setZonaColumnas] = useState<InboundZonaColumnaDinamica[]>([]);
  ...
  const loadZonaColumnas = useCallback(async (zona: string) => {
    setColLoading(true);
    setColOrder([]);
    const { data: cols } = await supabase.from('costos_inbound_zona_columnas').select('*').eq('zona', zona).order('orden');
    const colArray = (cols ?? []) as InboundZonaColumnaDinamica[];
    setZonaColumnas(colArray);
    if (colArray.length > 0) {
      const colIds = colArray.map(c => c.id);
      const { data: cells } = await supabase.from('costos_inbound_zona_celdas').select('*').in('columna_id', colIds);
    ...
    const savedKey = `costos-inbound-zone-order-${zona}`;
    try {
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    localStorage.setItem(`costos-inbound-zone-order-${activeZone}`, JSON.stringify(reordered));
    ...
    for (let i = 0; i < dynCols.length; i++) {
      ...
      if (col && col.orden !== i) {
        await supabase.from('costos_inbound_zona_columnas').update({ orden: i }).eq('id', dynCols[i]);
      }
    }
  ...
  const handleDeleteColumn = useCallback(async (colId: string) => {
    ...
    await supabase.from('costos_inbound_zona_celdas').delete().eq('columna_id', colId);
    await supabase.from('costos_inbound_zona_columnas').delete().eq('id', colId);
  ...
  const handleRenameColumn = useCallback(async (colId: string, nombre: string) => {
    ...
    await supabase.from('costos_inbound_zona_columnas').update({ nombre }).eq('id', colId);
  }, []);

  const handleOpenColumnFormulaEditor = useCallback((col: InboundZonaColumnaDinamica, e: React.MouseEvent) => {
  ...
  const handleSaveColumnFormula = useCallback(async (formula: string) => {
    ...
    await supabase.from('costos_inbound_zona_columnas').update({ formula: formula || null }).eq('id', columnaId);
    setEditingColumnFormula(null);
      ...
      {activeZoneData && (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="relative flex-1 min-w-[200px]">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><div className="w-3.5 h-3.5 flex items-center justify-center"><i className="ri-search-line text-sm text-slate-400" /></div></div>
              <input type="text" placeholder={`Buscar artículo en ${activeZone}...`} value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-100 focus:border-emerald-300 outline-none bg-white text-slate-700 placeholder:text-slate-400" />
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <select value={companiaFilter} onChange={e => setCompaniaFilter(e.target.value)} className="px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-100 focus:border-emerald-300 outline-none bg-white text-slate-700 min-w-[160px]">
                <option value="all">Todas las compañías</option>
                {companiasUnicas.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <ExportMenu
                filenameBase={`inbound-zona-${activeZone}`}
                getExportData={() => {
          ...
          {(zoneSwitching || isPending) && (
            <div className="relative"><div className="absolute inset-0 z-20 bg-white/80 backdrop-blur-sm rounded-lg flex items-center justify-center" style={{ minHeight: 200 }}><div className="flex flex-col items-center gap-3"><div className="w-10 h-10 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" /><p className="text-sm font-semibold text-slate-700">Cargando <strong className="text-emerald-600">{activeZone}</strong>...</p><p className="text-xs text-slate-400">Preparando datos y columnas dinámicas</p></div></div></div>
          )}

          {colLoading && !zoneSwitching && !isPending && (
            <div className="flex items-center gap-3 px-4 py-2.5 bg-emerald-50 border border-emerald-200 rounded-lg mb-2"><div className="w-5 h-5 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin flex-shrink-0" /><span className="text-sm font-medium text-emerald-700">Cargando columnas de <strong>{activeZone}</strong>...</span></div>
          )}
              ...
              <thead>
                <tr className="bg-slate-50 sticky top-0 z-10">
                  <SortableContext items={columnOrder} strategy={horizontalListSortingStrategy}>
                    {columnOrder.map(colKey => {
                      if (colKey.startsWith('FIXED:')) {
                        ...
                        if (!col) return null;
                        return <InboundSortableColHeader key={col.id} col={col} onDelete={handleDeleteColumn} onEditFormula={handleOpenColumnFormulaEditor} onRename={handleRenameColumn} onSort={() => toggleArtSort(colKey)} sortIconClass={sortIcon(colKey)} />;
                      }
                    })}
                    {colLoading && zonaColumnas.length === 0 && (
                      <th className="px-3 py-2.5 bg-slate-50 border-r border-slate-100">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="w-4 h-4 border-2 border-emerald-400 border-t-transparent rounded-full animate-spin ml-1" />
                        </div>
                      </th>
                    )}
                  </SortableContext>
                  <th className="px-1 py-2.5 bg-slate-50">
                    {colLoading ? (
                      <div className="flex items-center justify-center px-2"><div className="w-5 h-5 border-2 border-emerald-400 border-t-transparent rounded-full animate-spin" /></div>
                    ) : addingColumn ? (
                      <div className="flex items-center gap-1 px-1">
                        <input type="text" value={newColName} onChange={e => setNewColName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleAddColumn(); if (e.key === 'Escape') { setAddingColumn(false); setNewColName(''); } }} placeholder="Nombre..." className="w-[120px] px-2 py-1 text-xs border border-emerald-300 rounded-md focus:outline-none focus:ring-1 focus:ring-emerald-400 bg-white" autoFocus />
                        <button onClick={handleAddColumn} disabled={!newColName.trim()} className="w-6 h-6 flex items-center justify-center rounded-md bg-emerald-500 hover:bg-emerald-600 text-white cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><i className="ri-check-line text-xs" /></button>
              ...
              <tbody>
                {paginatedArts.length === 0 ? (
                  ...
                  paginatedArts.map((art, ai) => {
                    ...
                    const pctPromUnid = sumAllPromUnidArts > 0 ? (proms.promUnid / sumAllPromUnidArts) * 100 : 0;
                    return (
                      <tr key={`${activeZone}-${art.idCompania}-${art.articulo}`} className={`border-t border-slate-100 hover:bg-emerald-50/50 ${ai % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                        {columnOrder.map(colKey => {
                          if (colKey.startsWith('FIXED:')) {
                            const key = colKey.slice(6);
                            switch (key) {
                            ...
                            return (
                              <td key={col.id} onClick={(e) => handleOpenColumnFormulaEditor(col, e)} className={`px-3 py-2 text-right border-r border-slate-100 cursor-pointer transition-colors ${hasFormula ? 'hover:bg-emerald-100/60' : 'hover:bg-slate-100'}`} title={hasFormula ? cell?.formula : 'Clic para agregar fórmula'}>
                                {hasFormula ? (cell?.error ? <span className="text-rose-500"><i className="ri-error-warning-line mr-1" />Error</span> : isGlobalFormula ? <span className="text-slate-300 text-[10px] italic">—</span> : cell?.value !== null && cell?.value !== undefined ? <span className="text-emerald-700 font-medium tabular-nums">{new Intl.NumberFormat('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(cell.value)}</span> : <span className="text-slate-300">—</span>) : <span className="text-slate-300 text-[10px]">—</span>}
                              </td>
                            );
                          }
                        })}
...
function InboundSortableColHeader({ col, onDelete, onEditFormula, onRename, onSort, sortIconClass }: { col: InboundZonaColumnaDinamica; onDelete: (id: string) => void; onEditFormula: (col: InboundZonaColumnaDinamica, e: React.MouseEvent) => void; onRename: (id: string, nombre: string) => void; onSort: () => void; sortIconClass: string }) {
  ...
  return (
    <th ref={setNodeRef} style={style} className={`px-2 py-2.5 border-r font-semibold ${hasFormula ? 'bg-emerald-100/60 border-emerald-200' : 'bg-emerald-50/50 border-emerald-100'}`}>
      <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-1 min-w-0 flex-1">
          {editing ? (
            <input type="text" value={name} onChange={e => setName(e.target.value)} onBlur={handleSave} onKeyDown={e => { if (e.key === 'Enter') handleSave(); if (e.key === 'Escape') { setName(col.nombre); setEditing(false); } }} className="text-xs text-emerald-700 bg-white border border-emerald-300 rounded px-1.5 py-0.5 w-full min-w-[80px] focus:outline-none focus:ring-1 focus:ring-emerald-400" autoFocus />
          ) : (
            <div className="flex items-center gap-0.5 min-w-0 group/name">
              <span onClick={onSort} className="text-xs text-emerald-700 whitespace-nowrap max-w-[120px] overflow-hidden text-ellipsis cursor-pointer hover:underline">{col.nombre}</span>
              ...
              <button onClick={() => { setName(col.nombre); setEditing(true); }} className="w-4 h-4 flex items-center justify-center rounded text-slate-300 hover:text-emerald-500 hover:bg-emerald-100 cursor-pointer flex-shrink-0 opacity-0 group-hover/name:opacity-100 transition-opacity" title="Renombrar columna"><i className="ri-pencil-line text-[10px]" /></button>
            </div>
          )}
          {hasFormula && <span className="text-[10px] px-1 py-0.5 rounded bg-emerald-200 text-emerald-700 font-mono font-bold flex-shrink-0">fx</span>}
        </div>
        <button onClick={(e) => onEditFormula(col, e)} className={`w-5 h-5 flex items-center justify-center rounded cursor-pointer flex-shrink-0 transition-colors ${hasFormula ? 'text-emerald-600 hover:text-emerald-800 hover:bg-emerald-200' : 'text-slate-400 hover:text-emerald-500 hover:bg-emerald-100'}`} title={hasFormula ? 'Editar fórmula de columna' : 'Agregar fórmula a columna'}><i className={`${hasFormula ? 'ri-pencil-line' : 'ri-functions'} text-xs`} /></button>
