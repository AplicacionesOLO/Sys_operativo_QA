...
import { parseInboundMasivoExcel } from '@/lib/inboundMasivoExcelParser';
import type { MasivoParseResult } from '@/lib/inboundMasivoExcelParser';
...
export default function InboundExcelUploadModal({ onClose, onSuccess }: ExcelUploadModalProps) {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    const result = parseInboundMasivoExcel(buffer);
    setParsed(result);
    setStep('preview');
  }, []);
  ...
  const handleUpload = async () => {
    ...
    try {
      for (let i = 0; i < parsed.batches.length; i++) {
        ...
        const { error } = await supabase.from('costos_inbound_raw').insert(
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-4 max-h-[90vh] flex flex-col overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-base font-semibold text-slate-800">Cargar datos masivos - Inbound</h2>
          ...
          {step === 'select' && (
            ...
            >
              <div className="w-14 h-14 flex items-center justify-center rounded-full bg-emerald-50">
                <i className="ri-database-2-line text-3xl text-emerald-500" />
              </div>
              ...
              {!hasErrors && parsed && (
                <div className="space-y-3">
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 flex items-start gap-3">
                    ...
                    <p className="text-xs text-amber-700"><strong>Importante:</strong> Esta carga <strong>reemplazará todos los datos masivos anteriores</strong> de Inbound. Se subirán {parsed.batches.length} lotes de hasta 1.000 filas cada uno.</p>
                  </div>
                </div>
              )}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between gap-3 flex-shrink-0">
          ...
          {step === 'select' && (
            <>
              ...
              <button onClick={() => inputRef.current?.click()} className="px-5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">Seleccionar archivo</button>
            </>
          )}
          {step === 'preview' && (
            <>
              ...
              <button disabled={hasErrors} onClick={handleUpload} className="px-5 py-2 bg-emerald-500 hover:bg-emerald-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-medium rounded-lg transition-colors cursor-pointer whitespace-nowrap">Confirmar y subir</button>
            </>
          )}
          {step === 'done' && (
