...
import type {
  InboundArticuloResumenRow,
  InboundZonaResumenRow,
  InboundZonaArticuloDetalleRow,
  InboundZonaArticuloCompaniaRow,
  InboundZonaArticuloMensualRow,
  InboundResumenCompleto,
} from '@/types/costos_inbound';

// ── Re-export types for convenience ────────────────────────────────────────
export type {
  InboundArticuloResumenRow as ArticuloResumenRow,
  InboundZonaResumenRow as ZonaResumenRow,
  InboundZonaArticuloDetalleRow as ZonaArticuloDetalleRow,
  InboundZonaArticuloCompaniaRow as ZonaArticuloCompaniaRow,
  InboundZonaArticuloMensualRow as ZonaArticuloMensualRow,
  InboundResumenCompleto as ResumenCompleto,
};

// ── Masivo Resumen (stats básicos) ────────────────────────────────────────

export interface InboundMasivoResumen {
  totalRegistros: number;
  headers: string[];
}

export function useInboundMasivoResumen() {
  const [data, setData] = useState<InboundMasivoResumen | null>(null);
  const [loading, setLoading] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    const { count } = await supabase.from('costos_inbound_raw').select('*', { count: 'exact', head: true });
    if (!count || count === 0) { setData(null); setLoading(false); return; }
    const { data: sampleRow } = await supabase.from('costos_inbound_raw').select('raw_data').limit(1).single();
    let headers: string[] = [];
...
export function useInboundArticuloResumen() {
  const [data, setData] = useState<InboundResumenCompleto | null>(null);
  const [loading, setLoading] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let offset = 0;
      while (true) {
        const { data: page } = await supabase.rpc('fn_inbound_articulo_resumen', { p_offset: offset, p_limit: PAGE });
      ...
      const [{ data: zonasRaw }, { data: totalesRaw }, { count: totalRows }] = await Promise.all([
        supabase.rpc('fn_inbound_zona_resumen'),
        supabase.rpc('fn_inbound_totales'),
        supabase.from('costos_inbound_raw').select('*', { count: 'exact', head: true }),
      ]);
...
export function useInboundZonaCompaniaResumen(zona: string) {
  ...
  const load = useCallback(async () => {
    ...
    let offset = 0;
    while (true) {
      const { data: page } = await supabase.rpc('fn_inbound_zona_compania_articulo', { p_zona: zona, p_offset: offset, p_limit: PAGE });
      if (!page || page.length === 0) break;
...
export function useInboundZonaArticuloMensual(zona: string) {
  ...
  const load = useCallback(async () => {
    ...
    let offset = 0;
    while (true) {
      const { data: page } = await supabase.rpc('fn_inbound_zona_articulo_mensual', { p_zona: zona, p_offset: offset, p_limit: PAGE });
      if (!page || page.length === 0) break;
      allRows = allRows.concat(page);
      if (page.length < PAGE) break;
      offset += PAGE;
...
export function InboundRawTable({ headers }: { headers: string[] }) {
  ...
  const loadPage = useCallback(async (p: number) => {
    setLoading(true);
    const { data, count: c } = await supabase.from('costos_inbound_raw').select('id, raw_data', { count: 'exact' }).order('created_at', { ascending: false }).range(p * PAGE_SIZE, (p + 1) * PAGE_SIZE - 1);
    if (data) {
