...
import type {
  CrossdockingArticuloResumenRow,
  CrossdockingZonaResumenRow,
  CrossdockingZonaArticuloDetalleRow,
  CrossdockingZonaArticuloCompaniaRow,
  CrossdockingZonaArticuloMensualRow,
  CrossdockingResumenCompleto,
} from '@/types/costos_crossdocking';

// ── Re-export types for convenience ────────────────────────────────────────
export type {
  CrossdockingArticuloResumenRow as ArticuloResumenRow,
  CrossdockingZonaResumenRow as ZonaResumenRow,
  CrossdockingZonaArticuloDetalleRow as ZonaArticuloDetalleRow,
  CrossdockingZonaArticuloCompaniaRow as ZonaArticuloCompaniaRow,
  CrossdockingZonaArticuloMensualRow as ZonaArticuloMensualRow,
  CrossdockingResumenCompleto as ResumenCompleto,
};

// ── Masivo Resumen (stats básicos) ────────────────────────────────────────

export interface CrossdockingMasivoResumen {
  totalRegistros: number;
  headers: string[];
}

export function useCrossdockingMasivoResumen() {
  const [data, setData] = useState<CrossdockingMasivoResumen | null>(null);
  const [loading, setLoading] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    const { count } = await supabase.from('costos_crossdocking_raw').select('*', { count: 'exact', head: true });
    if (!count || count === 0) { setData(null); setLoading(false); return; }
    const { data: sampleRow } = await supabase.from('costos_crossdocking_raw').select('raw_data').limit(1).single();
    let headers: string[] = [];
...
export function useCrossdockingArticuloResumen() {
  const [data, setData] = useState<CrossdockingResumenCompleto | null>(null);
  const [loading, setLoading] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let offset = 0;
      while (true) {
        const { data: page } = await supabase.rpc('fn_crossdocking_articulo_resumen', { p_offset: offset, p_limit: PAGE });
      ...
      const [{ data: zonasRaw }, { data: totalesRaw }, { count: totalRows }] = await Promise.all([
        supabase.rpc('fn_crossdocking_zona_resumen'),
        supabase.rpc('fn_crossdocking_totales'),
        supabase.from('costos_crossdocking_raw').select('*', { count: 'exact', head: true }),
      ]);
...
export function useCrossdockingZonaCompaniaResumen(zona: string) {
  const [data, setData] = useState<CrossdockingZonaArticuloCompaniaRow[] | null>(null);
  const [loading, setLoading] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    let offset = 0;
    while (true) {
      const { data: page } = await supabase.rpc('fn_crossdocking_zona_compania_articulo', { p_zona: zona, p_offset: offset, p_limit: PAGE });
      if (!page || page.length === 0) break;
...
export function useCrossdockingZonaArticuloMensual(zona: string) {
  const [data, setData] = useState<CrossdockingZonaArticuloMensualRow[] | null>(null);
  const [loading, setLoading] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    let offset = 0;
    while (true) {
      const { data: page } = await supabase.rpc('fn_crossdocking_zona_articulo_mensual', { p_zona: zona, p_offset: offset, p_limit: PAGE });
      if (!page || page.length === 0) break;
      allRows = allRows.concat(page);
      if (page.length < PAGE) break;
      offset += PAGE;
...
export function CrossdockingRawTable({ headers }: { headers: string[] }) {
  ...
  const loadPage = useCallback(async (p: number) => {
    setLoading(true);
    const { data, count: c } = await supabase.from('costos_crossdocking_raw').select('id, raw_data', { count: 'exact' }).order('created_at', { ascending: false }).range(p * PAGE_SIZE, (p + 1) * PAGE_SIZE - 1);
    if (data) {
      setRows(data as Array<{ id: string; raw_data: Record<string, unknown> }>);
      setCount(c ?? 0);
    }
    setLoading(false);
  }, []);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <ExportMenu
          filenameBase="datos-crossdocking"
          ...
          <thead>
            <tr className="bg-slate-50 sticky top-0 z-10">
              ...
              {displayHeaders.map(h => (
                <th key={h} className="px-3 py-2 text-left text-slate-500 font-medium border-r border-slate-200 max-w-[200px] overflow-hidden text-ellipsis">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={displayHeaders.length + 1} className="px-3 py-8 text-center text-slate-400"><div className="w-5 h-5 border-2 border-teal-500 border-t-transparent rounded-full animate-spin mx-auto mb-2" />Cargando...</td></tr>
            ) : rows.length === 0 ? (
...
export function useCrossdockingClusterCompaniaResumen(zonas: string[]) {
  const [data, setData] = useState<CrossdockingZonaArticuloCompaniaRow[] | null>(null);
  const [loading, setLoading] = useState(false);
  const load = useCallback(async () => {
    ...
    while (true) {
      const { data: page } = await supabase.rpc('fn_crossdocking_zonas_compania_articulo', { p_zonas: zonas, p_offset: offset, p_limit: PAGE });
      if (!page || page.length === 0) break;
